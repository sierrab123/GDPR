...
function is_allowed_cookie( $cookie_name ) {
	if ( isset( $_COOKIE['gdpr']['allowed_cookies'] ) ) { // WPCS: input var ok.
		$allowed_cookies = json_decode( sanitize_text_field( wp_unslash( $_COOKIE['gdpr']['allowed_cookies'] ) ), true ); // WPCS: input var ok.
...
function has_consent( $consent ) {
	...
	if ( is_user_logged_in() ) {
		...
		$consents = array_map( 'sanitize_text_field', (array) json_decode( sanitize_text_field( wp_unslash( $_COOKIE['gdpr']['consent_types'] ) ) ) ); // WPCS: input var ok.
	}

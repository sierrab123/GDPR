...
class GDPR_Public {
	...
	public static function add_recaptcha() {
		...
		if ( $use_recaptcha ) {
			...
			if ( $site_key && $secret_key ) {
				echo '<div class="g-recaptcha" data-sitekey="' . esc_html( $site_key ) . '"></div>';
			}
		}
	}
	...
	public function privacy_preferences_modal() {
		...
		$user_consents          = isset( $_COOKIE['gdpr']['consent_types'] ) ? json_decode( sanitize_text_field( wp_unslash( $_COOKIE['gdpr']['consent_types'] ) ) ) : array(); // WPCS: input var ok.
	...
	public function update_privacy_preferences() {
		...
		$all_cookies = isset( $_POST['all_cookies'] ) ? array_map( 'sanitize_text_field', (array) json_decode( sanitize_text_field( wp_unslash( $_POST['all_cookies'] ) ) ) ) : array(); // WPCS: input var ok.
	...
	public function set_plugin_cookies() {
		...
		if ( ! isset( $_COOKIE['gdpr']['consent_types'] ) ) { // WPCS: input var ok.
			...
			if ( $user_id ) {
				...
				$cookie_consents = (array) json_decode( sanitize_text_field( wp_unslash( $_COOKIE['gdpr']['consent_types'] ) ) ); // WPCS: input var ok.
